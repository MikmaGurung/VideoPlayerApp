...
interface MetadataReader {
    fun getMetadataFromUri(uri: Uri): VideoItem?
    fun getMediaVideosFlow(
        selection: String? = null,
        selectionArgs: Array<String>? = null,
        sortOrder: String? = "${MediaStore.Video.Media.DISPLAY_NAME} ASC"): Flow<List<VideoItem>>
}
class MetadataReaderImpl(
    val app : Application
): MetadataReader{
    override fun getMetadataFromUri(uri: Uri): VideoItem? {
        var displayName : String?= null

        if (uri.scheme == "content"){
            Log.d(TAG,"Uri scheme is content")
        app.contentResolver.query(uri,null,null,null)?.use {
            it.moveToFirst()
            displayName = it.getStringOrNull(it.getColumnIndex(OpenableColumns.DISPLAY_NAME))
        }
        }
        else {
    ...
    override fun getMediaVideosFlow(
        ...
        app.contentResolver.registerContentObserver(VIDEO_COLLECTION_URI, true, observer)
        // initial value
        trySend(getMediaVideo(selection, selectionArgs, sortOrder))
        // close
